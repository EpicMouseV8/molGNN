...
def train_doublegraph(loader, model, optimizer, loss_fn, num_targets, device):
    model.train()
    total_loss = 0
    for data in loader:
        data = data.to(device)
...
def evaluate_doublegraph(loader, model, loss_fn, num_targets, device):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for data in loader:
...
def run_doublegraph_training(data, epochs, target_names, model_name):
    ...
    model = DoubleGraphGNN(node_feature_dim, edge_feature_dim, output_dim)
    ...
    for epoch in range(epochs):
        train_loss = train_doublegraph(train_loader, model, optimizer, loss_fn, num_targets, device)
        val_loss = evaluate_doublegraph(val_loader, model, loss_fn, num_targets, device)
        if epoch % 100 == 0:
    ...
    test_model(model, test_loader, num_targets, target_names)
